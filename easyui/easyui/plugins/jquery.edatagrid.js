			...
			onAfterEdit: function(index, row){
				...
				if (url){
					$.post(url, row, function(result){
						if (result.state==0){
							 $.messager.alert(opts.destroyMsg.norecord.title,result.message,'error');
							 return false;
				        }else{
							$.messager.show({title: opts.tipMsgTitle, showType:'slide', showSpeed:500, msg: result.message,timeout:2500});
						}
						result.data.isNewRecord = null;
				...
				$.messager.confirm(opts.destroyMsg.confirm.title,opts.destroyMsg.confirm.msg,function(r){
					if (r){
						...
						if (row.isNewRecord){
							...
							if (opts.destroyUrl){
								var idValue = row[opts.idField||'id'];
								$.post(opts.destroyUrl, {id:idValue}, function(result){
									if (result.state==0){
										 $.messager.alert(opts.destroyMsg.norecord.title,result.message,'error');
										 return false;
									}else{
