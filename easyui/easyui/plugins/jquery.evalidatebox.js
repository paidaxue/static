$.extend($.fn.validatebox.defaults.rules, {
	...
	isurl:{// 验证url格式
		validator: function (value) {
			value		= $.trim(value);
			return /^((http|ftp|https)\:\/\/)+(www.|[a-zA-Z].)[a-zA-Z0-9\-\.]+\.(com|cn|edu|gov|mil|net|org|biz|info|name|museum|us|ca|uk)(\:[0-9]+)*(\/($|[a-zA-Z0-9\.\,\;\?\'\\\+&amp;%\$#\=~_\-]+))*$/i.test(value);
		},
		message: '链接地址格式不正确.'
	},
	minLength: {
	...
	mobilePhone: {// 验证固话或手机号码
		validator: function (value) {
			value		= $.trim(value);
			var isPhone = /^([0-9]{3,4}-)?[0-9]{7,8}$/;
		    var isMob=/^(13|14|15|17|18)\d{9}$/;
		    if(isMob.test(value)||isPhone.test(value)){
		        return true;
		    } 
		    else{
		    	message: '手机号码或固话格式不正确'
		    }
		},
		message: '手机号码或固话格式不正确'
	},
	mobile: {// 验证手机号码
		...
		message: '请使用格式:020-88888888或手机号码'
	},
	intOrFloat: {// 验证整数或小数
		validator: function (value) {
	...
	moneyE: {// 验证整数或小数
		validator: function (value) {
			return /^\d+(\.\d{1,5})?$/i.test(value);
		},
		message: '请输入数字，小数最多5位'
	},
	currency: {// 验证货币
		validator: function (value) {
			return /^\d+(\.\d+)?$/i.test(value);
	...
	capitalNum: {// 验证大写字母+数字组合
		validator: function (value) {
			return /^[A-Z][0-9]{4,15}$/.test(value);
		},
		message: '请输入5到16个大写字母+数字组合'
	},
	...
	username: {// 验证用户名
		...
		message: '用户名不合法（请输入5到16个以字母开头的数字或字母）'
	},
	loginname: {// 验证用户名
		validator: function (value) {
			value		= $.trim(value);
	...
	sku:{
		validator: function (value) {
			valueLen		= $.trim(value).length;
			value		= $.trim(value);
			if (valueLen < 5 || valueLen> 40){
	            return false; 
	        }
			//return /^[A-Za-z0-9_-]{3,40}$/.test(value);
			return /^[0-9a-zA-Z]+([0-9a-zA-Z]+|(-|_)?)+[0-9a-zA-Z]+$/.test(value);
		},
		message: '5到30位的字母数字-_,不能以-_结尾,-_只能各使用一次'
	},
	selectValueRequired:{ 
		validator: function(value,param){ 
		//console.info($(param[0]).find("option:contains('"+value+"')").val());
		return $(param[0]).find("option:contains('"+value+"')").val() != '请选择'; 
		}, 
		message: '该输入项为必输项' 
	} //在需要验证的select加上 data-options="required:true,validType:'selectValueRequired[\'#selectID\']'"
}); 